...
L12=koff*speye(N^2);
L21=kon*speye(N^2);
L11=-kb*spdiags(Irn,0,N^2,N^2)-L21;
L22=-kb*spdiags(Irn,0,N^2,N^2)-L12;
tic
...
b2=B22;
kIrnbar=-kb*mean(Irn);
RM11=kIrnbar+(((koff.*conj(v11).*v21)+(kon.*conj(v21).*v11)-(((D1.*v11.^2)+(D2.*v21.^2)).*w12m2)-(koff.*v21.^2)-(kon.*v11.^2))./(v11.^2+v21.^2));
M11=kIrnbar+(((koff.*conj(v12).*v22)+(kon.*conj(v22).*v12)-(((D1.*v12.^2)+(D2.*v22.^2)).*w12m2)-(koff.*v22.^2)-(kon.*v12.^2))./(v12.^2+v22.^2));
Rlam1=RM11;
lam1=M11;
p=L11*u2+koff*b2;
q=kon*u2+L22*b2;  %   Q=[p;q];  %Q=L*B
p2=reshape(p,N,N);
q2=reshape(q,N,N);
Rlam2=kIrnbar;
lam2=Rlam1;
